...
#wandbLogDict={}
groupName="LLMIntegration_AuxLossAndActionMask"
...
class Agent:
    ...
    def learn(self, trainingData, recommendedDists=np.array([]), probabilityModifier=None):
        ...
        for epoch in range(self.epochs):
            ...
            for batch in batches:
                ...
                if recommendedDists:
                    ...
                    # print("AGENT PROBS SHAPE: ", agentProbs.shape)
                    # print("LLM PROBS SHAPE: ", llmProbs.shape)
        ...
        #For calculating explaiend variance (how good critic is at predicting values)
...
class PlayGame:
    def __init__(self,numEps:int=100,numSteps:int=32,batchSize:int=64,trgInterval=256,
        ...
        #path=path/"ModifiedCybORG/scenario.yaml"
        #Original File Path
        # path:str = str(inspect.getfile(CybORG))
        # path:str = path[:-10] + '/Shared/Scenarios/Scenario2.yaml'
        # path=path/"ModifiedCybORG/modifiedScenarios/scenario_13hosts_3subnets_deterministic.yaml"
        # agents:dict={
        #     #'Red': B_lineAgent,
        #     'Red': B_lineAgent13Deterministic,
        #     #'Red': RedMeanderAgent,
        #     'Green': GreenAgent,
        # }
        path=path/"ModifiedCybORG/modifiedScenarios/scenario_13hosts_3subnets.yaml"
        agents:dict={
            #'Red': B_lineAgent,
            'Red': B_lineAgent13,
            #'Red': RedMeanderAgent,
    ...
    def saveGraph(self,totalTrgTime,epRewards, numEps, numSteps, \
                  savePath=f'/home/users/tholl/CageChallenge2LLM/ModifiedCybORG/figures/TuningGraph{int(time.time())}.png', \
    ...
    def getBestActionAndDist(self,observation):
        ...
        # print("LLM RECOMMENDED ACTION: ", str(self.env.possibleActions[recommendedAction]))
        # print("SAMPLED ACTION: ", str(self.env.possibleActions[action]))
        
        # recommendedActionDist={str(possibleAction):prob for possibleAction,prob in zip(self.env.possibleActions, dist)}

        # print("RECOMMENDED ACTION DIST: ", recommendedActionDist)

        return action,dist.detach()#dist.probs.detach()
    
    def playRound(self, useTrueState=False, saveIteration=0):
        ...
        #Uncomment for aux loss only
        probabilityModifier=None

        bestScore=-40 #Best score
        lastEpReward=-10000 #Last episode reward
        lastActions=[] #just for debugging
        startTime=time.time() #Calculate how long training takes
        # path:str = str(inspect.getfile(CybORG))
        # path:str = path[:-10] + '/Shared/Scenarios/Scenario2.yaml'
        #Load custom scenario file (using pathLib to get the path of the file)
        #actionSize=self.env.actionSpace.n
        ...
        self.preTrainedRLAgent.load_checkpoint(f'/home/users/tholl/CageChallenge2LLM/ModifiedCybORG/newStableImplementation/LLMDistillations/DistLLMDistilled_0.5Temp.pth')
        ...
        for ep in range(0,self.numEps):
            ...
            for step in range(0, self.numSteps):
                ...
                if not probabilityModifier and ppoAgent.pretrainedScale < 1:
                    # print("PRETRAINED SCALE: ", ppoAgent.pretrainedScale)
                    # self.recommendedAction=self.env.getLLMRecommendation()
                    self.recommendedAction,self.recommendedDist=self.getBestActionAndDist(obs)
                    recommendedDists.append(self.recommendedDist) 
                ...
                # prob = torch.squeeze(maskedProbs.log_prob(action)).item()

                action = torch.squeeze(action).item()
                # print("SELECTED ACTION: ", action)
                # print("TEACHER'S RECOMMENDATION: ", self.recommendedAction)  
                # if self.recommendedAction: action=self.recommendedAction
                ...
                # reward=baseline+currentReward
                # if reward < worstScore:
                #     worstScore=reward
                #     print(f"Worst score: {worstScore}")
                # reward+=15
                # reward = ((reward + 20) / 20) * 10
            ...
            # # #If the episode is at threshold, save the model!
            # epSaveNumber=100
            # if ep==epSaveNumber:
            #     ppoAgent.savePolicyModel(f'{str(Path(__file__).parent)}/baseline{epSaveNumber}_Iteration{saveIteration}.pth')
            # elif ep > epSaveNumber:
            #     break


            #Save policy model for distillation!
            if ep >= 503:
                ppoAgent.savePolicyModel(f'/home/users/tholl/CageChallenge2LLM/ModifiedCybORG/newStableImplementation/LLMDistillations/StandardDist_0.5Temp.pth')
...
if __name__=="__main__":
    ...
    # newGame=PlayGame(numEps=20000, llmHelp=True, llmPromptType="sentenceFormat", batchSize=batchSize,trgInterval=trgInterval,\
    #                  policyLR=lr,valueLR=criticLR,policyClip=policyClip, criticGradClip=criticGradClip)
    ...
    for y in range(0,1):
        groupName=f"LLMOptimizedDist_0.5Temp{y}_DecayAfter1"
        for i in range(0,1):
            ...
            startTime=time.time()
            rewards.append(newGame.playRound(saveIteration=i))
            #wandb.finish()
    averageRewards=np.mean(np.array(rewards),axis=0)
    newGame.saveGraph(time.time()-startTime,averageRewards, 300, 32, title=f"tmpHyper", savePath=f"newPPO_LLMStandardDist0.5Temp.png")