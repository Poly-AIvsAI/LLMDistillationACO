...
class Agent:
    def __init__(self, actionSize, stateSize, gamma=0.99, policyLR=0.0003, criticLR=0.0003, gaeLambda=0.95,
            policyClip=0.1, epochs=10, entropyCoef=0.001, entropyDecay=0.99, policyGradClip=0.5, criticGradClip=0.1, startLLMDecay=4):
        self.gamma = gamma
        ...
        self.startLLMDecay=startLLMDecay
    ...
    def learn(self, trainingData, recommendedActions=np.array([]), probabilityModifier=None):
        criticLossArr=[]
        actorLossArr=[]
        criticValsArr=[]
        returnsArr=[]
        ...
        for epoch in range(self.epochs):
            ...
            for batch in batches:
                ...
                recommendedActionsBatch=torch.tensor(recommendedActions[batch]).to(self.actor.device) if recommendedActions.any() else []
                ...
                #Computing the ratio between the old policy and new policy to determine how much to update
                newProbs = dist.log_prob(actions)
                probRatio = newProbs.exp() / oldProbs.exp()
                #OR CAN DO:
                ...
                if recommendedActions.any():
                    ...
                    #Torch uses natural log
                    #Detaching auxWeight since it's just scalar (don't want it to track gradients and impact learning)
                    # auxWeight = actorLoss.abs().mean().detach()
                    # auxWeight=1
                    pretrainedProbs=dist.log_prob(recommendedActionsBatch)
                    pretrainedLoss=-pretrainedProbs.mean()

                    # pretrainedLoss=torch.clamp(pretrainedLoss*auxWeight,max=actorLoss.abs().mean().detach())
                   
                    #Ensure that pretrained loss doesn't exceed actor loss
                    #If prob is very close to 0 could be very large (clip to limit exploding gradients)
                    # pretrainedLoss = torch.clamp(pretrainedLoss, min=-10, max=10)
                    totalLoss = (self.pretrainedScale)*actorLoss + (1-self.pretrainedScale)*pretrainedLoss + 0.5*criticLoss\
        ...
        # # # #Do extra epochs for the critic to have it catch up!
        # if self.pretrainedScale < 1:
        #     for epoch in range(self.epochs):
        ...
        #STANDARD
        # #Gradual decrease
        # if self.pretrainedScale < 1 and self.trgIntervalCounter >= self.startLLMDecay:
        #     self.pretrainedScale=min(1, self.pretrainedScale+0.25)
        #     self.entropyWeight+=0.0005
        # elif self.entropyWeight > 0.005 and self.trgIntervalCounter>=self.startLLMDecay:
        #     self.entropyWeight=max(0.005,self.entropyWeight-0.00025)
        #     # self.entropyWeight=max(0.005, self.entropyWeight-0.00025)
        # elif self.trgIntervalCounter>=self.startLLMDecay:
        #     self.entropyWeight*=self.entropyDecay

        # #Gradually decay by 0.25 as well!
        # if probabilityModifier and self.trgIntervalCounter >= self.startLLMDecay:
        ...
        #Gradually decay action masking as well!
        if probabilityModifier and self.trgIntervalCounter >= self.startLLMDecay:
            probabilityModifier.impactScale=min(1, probabilityModifier.impactScale+0.1)
        


        #Not sure why, but sometimes treats as 0.9999 instead of 1 when verified outside of object
        if self.pretrainedScale > 0.999999:
            self.pretrainedScale = 1
        self.trgIntervalCounter+=1
        # self.entropyWeight*=self.entropyDecay
        #print("NUMBER OF LOW AND HIGH CLIPS: ", numLowClips, " ", numHighClips)
            # self.actor.scheduler.step()
            #self.critic.scheduler.step()
        # print(self.actor.optimizer.param_groups[0]['lr'])
        # print(self.critic.optimizer.param_groups[0]['lr'])
...
class ProbabilityModifier:
    ...
    def modifyBatchProbabilities(self,dist,batch):
        ...
        for actionMaskingIndex in range(0,len(actionMaskings)):
            ...
            for actionIndex in range(0,len(self.possibleActions)):
                ...
                #Decreasing probabilities of hosts not recommended by LLM
                # if hostToActOn and hostToActOn not in str(tmpAction):
                #     actionMaskings[actionMaskingIndex][actionIndex]=1/self.impactScale
                #Increasing probabilities of hosts recommended by LLM
                #UNCOMMENT to include hosts recommended so far in episode to masking
                # tmpActionHostArray=str(tmpAction).split(' ')
                # hostInAction=tmpActionHostArray[1] if len(tmpActionHostArray)>1 else None
                # if hostToActOn and (hostToActOn not in str(tmpAction) and hostInAction not in self.hostsToNotMask):
    ...
    def modifyProbabilities(self,dist, recommendedAction=None):
        ...
        for actionIndex in range(0,len(self.possibleActions)):
            ...
            #Decreasing probabilities of hosts not recommended by LLM
            # if hostToActOn and hostToActOn not in str(tmpAction):
            #     actionMasking[actionIndex]=1/self.impactScale
            #Increasing probability of host recommended by LLM
             #UNCOMMENT to include hosts recommended so far in episode to masking
            # tmpActionHostArray=str(tmpAction).split(' ')
            # hostInAction=tmpActionHostArray[1] if len(tmpActionHostArray)>1 else None
            # if hostToActOn and (hostToActOn not in str(tmpAction) and hostInAction not in self.hostsToNotMask):
...
class PlayGame:
    def __init__(self,numEps:int=100,numSteps:int=32,batchSize:int=64,trgInterval=256,
               ...
               entropyCoef=0.005, entropyDecay=0.99, useTrueState=False,saveModelPath=False,
               saveToDB=False,hyperParameterTuning=False,llmHelp=None,llmPromptType="json",startLLMDecay=4):
        self.startLLMDecay=startLLMDecay
        ...
        self.env:CustomBlueWrapper=CustomBlueWrapper(env=self.cyborg,useLLM=llmHelp, llmPromptType=llmPromptType)
        ...
        #Dr. Ranwa's recommendation of using pretrained RL agent instead of LLM for feature space modification
        # self.preTrainedRLAgent=ActorNetwork(actionDim=79,stateDim=91,learnRate=self.policyLR)
        # self.preTrainedRLAgent.load_checkpoint(f"{str(Path(__file__).parent)}/newBest.pth")

    #Method to get action from pretrained RL agent (Dr. Ranwa's suggestion of using RL agent for feature space modification)
    def getPretrainedAction(self,observation):
        state = torch.tensor([observation], dtype=torch.float).to(self.device)
        dist = self.preTrainedRLAgent(state)
        action = dist.sample()
        action = torch.squeeze(action).item()
        return action
    
    #Method to get the best host from the pretrained RL Agent
    def getBestHost(self,observation):
        action=self.getPretrainedAction(observation)
        actionAsString=str(self.env.possibleActions[action])
        chosenHost="DNE"
        possibleHosts=[host for host in self.env.blueInfo if host != "success"]
        for host in possibleHosts:
            if host in actionAsString:
                chosenHost=host
                break
        return chosenHost
    ...
    def playRound(self, loadPath, useTrueState=False, saveIteration=0):
        ...
        self.preTrainedRLAgent=ActorNetwork(actionDim=actionSize,stateDim=stateSize,learnRate=self.policyLR)
        # self.preTrainedRLAgent.load_checkpoint(f'{str(Path(__file__).parent)}/baseline100_Iteration2.pth')
        self.preTrainedRLAgent.load_checkpoint(loadPath) #Loading distilled standard
        ...
        ppoAgent = Agent(actionSize=actionSize,stateSize=stateSize,policyLR=self.policyLR,criticLR=self.valueLR,
                        policyClip=self.policyClip,epochs=self.epochs, entropyCoef=self.entropyCoef, entropyDecay=self.entropyDecay, \
                            policyGradClip=self.policyGradClip, criticGradClip=self.criticGradClip, startLLMDecay=self.startLLMDecay)
        ...
        for ep in range(0,self.numEps):
            ...
            for step in range(0, self.numSteps):
                ...
                #Get the best host for masking
                    ...
                    self.recommendedAction=self.getPretrainedAction(obs)
                    recommendedActions.append(self.recommendedAction) 
                ...
                #Convert reward to have more meaningful signals
                ...
                if totalSteps % self.trgInterval == 0:
                    ...
                    ppoAgent.learn(trainingData, recommendedActions=np.array(recommendedActions), probabilityModifier=probabilityModifier)
                    recommendedActions=[]
                    #Clear the data
                    trainingData=[[],[],[],[],[],[]]
                obs=nextObs
...
if __name__=="__main__":
    ...
    newGame=PlayGame(numEps=10000, llmHelp=False, llmPromptType=False, batchSize=batchSize,trgInterval=trgInterval,\
                     policyLR=lr,valueLR=lr,policyClip=policyClip, criticGradClip=criticGradClip, entropyCoef=entropyCoef)
    rewards=[]

    scenarios=["scenario_4hosts_3subnets.yaml","scenario_5hosts_3subnets.yaml","scenario_6hosts_3subnets.yaml","scenario_7hosts_3subnets.yaml",
               "scenario_8hosts_3subnets.yaml","scenario_9hosts_3subnets.yaml","scenario_10hosts_3subnets.yaml","scenario_11hosts_3subnets.yaml",
               "scenario_12hosts_3subnets.yaml"]
    blineAgents=[B_lineAgent4,B_lineAgent5,B_lineAgent6,B_lineAgent7,B_lineAgent8,B_lineAgent9,B_lineAgent10,B_lineAgent11,B_lineAgent12]
    standardDecayLLMImpactList=[1,1,1,1,2,2,2,3,3]
    optimizedDecayLLMImpactList=[10,10,10,15,20,20,20,25,30]

    llmHelp=False
    llmPromptType=False
    for y in range(0,len(scenarios)):
        groupName=f"OptimizedAuxAndActionMask{y}Hosts"
        path=Path(__file__).parent.parent
        path=path/f"CybORGModified/CybORG/Shared/Scenarios/modifiedScenarios/{scenarios[y]}"
        agents:dict={
            #'Red': B_lineAgent,
            'Red': blineAgents[y],
            #'Red': RedMeanderAgent,
        ...
        newGame.startLLMDecay=optimizedDecayLLMImpactList[y]
        #If don't specify agents in environment, they default to sleep agents
        cyborg:CybORG = CybORG(path,'sim',agents=agents)
        env:CustomBlueWrapper=CustomBlueWrapper(env=cyborg,useLLM=llmHelp, llmPromptType=llmPromptType,hostHops=None)
        newGame.env=env
        for i in range(0,10):
            # #wandb.init(project="ThesisDifferentNumHosts",group=groupName,name=f"run{i}",config={"Algorithm": groupName}, mode="offline", id=f"{groupName}_Run{i}_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M')}")
            startTime=time.time()
            rewards.append(newGame.playRound(loadPath=Path(__file__).parent.parent/f'Distillation/LLMDistillations/distilledLLMHost{y+4}.pth'))
            # #wandb.finish()